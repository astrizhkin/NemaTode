...
int main(int argc, char** argv){
	...
	gps.onUpdate += [&gps](NMEASentence::MessageID messageId){
		cout << (gps.fix.locked() ? "[*] " : "[ ] ") << setw(2) << setfill(' ') << gps.fix.trackingSatellites << "/" << setw(2) << setfill(' ') << gps.fix.gsvVisibleSatellites() << " ";
		cout << fixed << setprecision(2) << setw(5) << setfill(' ') << gps.fix.gsvAverageSNR() << " dB   ";
